...
class OldSupervisor extends \Socket\Server {
...
//                fclose($self->SELF_PIPE[0]); // close pipes and reinit them?
                     ...
                     * If we don't exit here we risk the supervisor
                     * running in 2 places!
                     */
                    exit;
                })){
    ...
    /**
     * Attempt to awaken the master by passing
     * writing a token to the master PIPE.
     */
    ...
    /**
     * Attempt to kill of all workers
     * with $signal
     * @param $signal
     */
    ...
    /**
     * Kill a worker with $pid using $signal
     * @param $pid
     * @param $signal
     */
    public function killWorker($pid, $signal){
        try {
            IO::write(STDOUT, "killing worker pid $pid with $signal.\n");
    ...
    /**
     * Reap all the currently running workers that have a
     ...
     * Providing $graceful = true attempts
     * to shut down each worker
     * @param bool $graceful
     */
    ...
    /**
     * Remove the work from the listed set
     * of workers currently running.
     * @param int $pid
     */
